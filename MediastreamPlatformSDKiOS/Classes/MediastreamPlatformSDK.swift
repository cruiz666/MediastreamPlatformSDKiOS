...
public class MediastreamPlatformSDK: UIViewController, IMAAdsLoaderDelegate, IMAAdsManagerDelegate {
    ...
    open var currentTime: Double {
        ...
        set (val) {
            if self.player == nil {
                ...
                let newTime = CMTimeMakeWithSeconds(val, preferredTimescale: 1)
    ...
    open func playBackgroundAudio() {
        do {
            if #available(iOS 10.0, *) {
                ...
                AVAudioSession.sharedInstance().perform(NSSelectorFromString("setCategory:error"), with: AVAudioSession.Category(rawValue: convertFromAVAudioSessionCategory(AVAudioSession.Category.playback)))
            }
    ...
    open func fordward(_ time: Double) {
        ...
        if newTime < (CMTimeGetSeconds(duration) - time) {
            let finalTime: CMTime = CMTimeMake(value: Int64(newTime*1000), timescale: 1000)
    ...
    open func preparePlayer(player: AVPlayer) {
        ...
        if (self.config?.customUI)! {
            ...
            if let customUI = mediastreamFramework.loadNibNamed("MediastreamCustomView", owner: self, options: nil)?.first as? MediastreamCustomUIView {
                ...
                customUI.slider.addTarget(self, action: #selector(customUISliderValueChanged), for: UIControl.Event.valueChanged)
        ...
        if self.playerTime != nil {
            let newTime = CMTimeMakeWithSeconds(self.playerTime!, preferredTimescale: 1)
            self.player?.seek(to: newTime, toleranceBefore: CMTime.zero, toleranceAfter: CMTime.zero)
        }
